...
> [!IMPORTANT]
> For npm to be able to publish make sure to register the action for [Trusted Publishing](https://docs.npmjs.com/trusted-publishers) with the package.
...
If a `release-npm` annotation is found in the body a release will be triggered. Patch, minor or major release type will be decided based on the commit history.
...
Using trusted publishers provenance will be generated by default without setting the flag in npm `publishConfig`.
