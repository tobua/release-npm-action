...
export default async function (npmrc, { npmPublish, pkgRoot }, pkg, context) {
  ...
  if (npmPublish !== false && pkg.private !== true) {
    ...
    console.log('publish', basePath, npmrc, readFileSync(npmrc, 'utf-8'))
    console.log(cwd, process.cwd(), env, path.resolve(process.cwd(), '.npmrc'))

    logger.log(`Publishing version ${version} to npm registry on dist-tag ${distTag}`);
    const result = execa(
      "npm",
      ["publish", basePath, "--userconfig", resolve(process.cwd(), '.npmrc'), "--tag", distTag, "--registry", registry, '--provenance'],
